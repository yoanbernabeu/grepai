...
func TestProjectPrefixStore_SaveChunks(t *testing.T) {
	...
	// The raw chunk ID is preserved verbatim (slash-normalized) under the
	// prefix instead of being rewritten from the file path.
	if mock.savedChunks[1].ID != wrapped.getPrefix()+"/orig_1" {
		t.Errorf("chunk1 id = %q, want %q", mock.savedChunks[1].ID, wrapped.getPrefix()+"/orig_1")
	}

	// IDs without an underscore are prefixed like any other raw ID.
	if mock.savedChunks[2].ID != wrapped.getPrefix()+"/plainid" {
		t.Errorf("chunk2 id = %q, want %q", mock.savedChunks[2].ID, wrapped.getPrefix()+"/plainid")
	}
}

// TestProjectPrefixStore_SaveChunksPreservesReChunkIDs covers multi-underscore
// IDs generated by Chunker.ReChunk ("file_parent_sub"): the writer must keep
// the entire raw ID so sub-chunks stay unique and resolvable, instead of
// collapsing to the segment after the last underscore.
func TestProjectPrefixStore_SaveChunksPreservesReChunkIDs(t *testing.T) {
	ctx := context.Background()
	projectRoot := t.TempDir()
	mock := &mockVectorStore{}
	wrapped := &projectPrefixStore{
	...
	relPath := filepath.Join("src", "nested", "a.go")
	relSlash := filepath.ToSlash(relPath)
	chunks := []store.Chunk{
		{ID: relSlash + "_0", FilePath: relPath},
		{ID: relSlash + "_0_0", FilePath: relPath},
		{ID: relSlash + "_0_1", FilePath: relPath},
		{ID: relSlash + "_1", FilePath: relPath},
	}
	if err := wrapped.SaveChunks(ctx, chunks); err != nil {
		t.Fatalf("SaveChunks failed: %v", err)
	}
	if len(mock.savedChunks) != len(chunks) {
		t.Fatalf("saved chunk count = %d, want %d", len(mock.savedChunks), len(chunks))
	}
	seen := make(map[string]string, len(chunks))
	for i, raw := range []string{relSlash + "_0", relSlash + "_0_0", relSlash + "_0_1", relSlash + "_1"} {
		wantID := wrapped.getPrefix() + "/" + raw
		if mock.savedChunks[i].ID != wantID {
			t.Errorf("chunk%d id = %q, want %q", i, mock.savedChunks[i].ID, wantID)
		}
		if prev, dup := seen[mock.savedChunks[i].ID]; dup {
			t.Errorf("chunk%d id %q collides with %q", i, mock.savedChunks[i].ID, prev)
		}
		seen[mock.savedChunks[i].ID] = raw
		wantPath := wrapped.getPrefix() + "/" + relSlash
		if mock.savedChunks[i].FilePath != wantPath {
			t.Errorf("chunk%d path = %q, want %q", i, mock.savedChunks[i].FilePath, wantPath)
		}
	}
}
